#import <Foundation/Foundation.h>
#import "AppInfo.h"

@protocol InstalltraceDelegate<NSObject>

@optional

/// 应用点击调起时，获取由web传递过来的动态参数（如邀请码、渠道等）和相关信息
///
/// appInfo 由web传递过来的动态参数
- (void)handleWakeUpTrace:(AppInfo * _Nullable)appInfo;

@end

@interface Installtrace : NSObject

/// 获取SDK当前的版本号
+ (nonnull NSString *)sdkVersion;

/// 初始化Installtrace SDK
/// 调用该方法前，需在Info.plist文件中配置键值对
/// <key>com.Installtrace.appKey</key>
/// <string>管理平台分配的key</string>
///
/// delegate 实现InstalltraceDelegate的对象实例
+ (void)initWithDelegate:(nullable id<InstalltraceDelegate>)delegate;

/// 初始化Installtrace SDK
///
/// delegate 实现InstalltraceDelegate的对象实例
/// appKey 管理平台分配的key
+ (void)initWithDelegate:(nullable id<InstalltraceDelegate>)delegate
                  appKey:(nonnull NSString *)appKey;

/// 获取由web传递过来的动态参数（如邀请码、渠道等）和相关信息
///
/// 默认回调超时时长为: 10秒
/// success 成功回调block，在主线程回调
/// fail 失败回调block，在主线程回调
+ (void)getInstallTrace:(void (^ _Nullable) (AppInfo * _Nullable appInfo))success
                   fail:(void (^ _Nullable) (NSInteger errCode, NSString * _Nonnull errMsg))fail;

/// 获取由web传递过来的动态参数（如邀请码、渠道等）和相关信息，可自定义回调超时时长
///
/// timeout 回调超时时长，单位为秒
/// success 成功回调block，在主线程回调
/// fail 失败回调block，在主线程回调
+ (void)getInstallTraceWithTimeout:(NSTimeInterval)timeoutSeconds
                           success:(void (^ _Nullable) (AppInfo * _Nullable appInfo))success
                              fail:(void (^ _Nullable) (NSInteger errCode, NSString * _Nonnull errMsg))fail;

/// 处理 URI Schemes 逻辑
///
/// url 通过Schemes调起时，系统回调回来的URL
/// return bool Installtrace是否成功识别该URL
+ (BOOL)handleSchemeLinkURL:(nullable NSURL *)url;

/// 处理 Universal link 逻辑
///
/// userActivity 通过Universal link调起时，包含系统回调回来的URL信息的NSUserActivity
/// return bool Installtrace是否成功识别该URL
+ (BOOL)handleUniversalLink:(nullable NSUserActivity *)userActivity;

/// 设置目标服务器的domain, 仅供私有化部署用户使用
///
/// 方式1: 如在代码中设置, 需要在getInstallTrace之前调用才能生效
/// 方式2: 在Info.plist文件中配置键值对
/// <key>com.Installtrace.serverDomain</key>
/// domain 目标服务器的domain, 如: https://api.Installtrace.com
+ (void)setServerDomain:(nonnull NSString *)domain;

/// 设置SDK不访问剪切板，若禁用会影响匹配成功率。默认情况下，SDK会访问剪切板，以获取剪切板中的动态参数
///
/// 需要initWithDelegate之前调用才能生效
+ (void)disableClipboard;

/// 判断是否首次请求getInstallTrace，只要请求成功过一次（success回调被调用过一次）则返回NO
///
/// return BOOL 是否首次请求
+ (BOOL)isFirstGetInstallTrace;

/// 开启Debug模式
///
/// 仅在开发阶段开启，开启后会记录更多调试日志
/// debug 是否开启Debug模式，默认为 NO
+ (void)setDebug:(BOOL)debug;

/// 写入日志，开启Debug模式后才会生效
///
/// log 日志信息
+ (void)appendLog:(nonnull NSString *)log;

/// 提取所有的调试日志
///
///return NSString 调试日志
+ (nullable NSString *)dumpLog;

@end


